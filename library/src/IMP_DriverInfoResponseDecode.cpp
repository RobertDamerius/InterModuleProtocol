#include "IMP_DriverInfoResponseDecode.h"
#include <InterModuleProtocol.hpp>
#include <cstring>


void IMP_CreateDriverInfoResponseDecode(void){}

void IMP_DeleteDriverInfoResponseDecode(void){}

void IMP_OutputDriverInfoResponseDecode(uint8_t* bytes, uint32_t length, int32_t* numDecodedBytes, double* timestamp, double* dimension, double* offset, uint8_t* textVehicleName, uint8_t* textDescription, uint32_t maxTextLength){
    *timestamp = 0.0;
    dimension[0] = 0.0;
    dimension[1] = 0.0;
    dimension[2] = 0.0;
